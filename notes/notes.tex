...
\frac{1}{2}\norm{\log.(P_{det} M_{shift} W \tilde{x}_{365})}^2 + \frac{1}{2}\lambda \norm{\log W}^2
.$$
The first term measures the average logarithm of the predicted values $\hat{y}$ (detection events only). The second term measures the average logarithm of the weights.
\begin{proposition}
    ...
    $$
    \mathrm{grad}_{W} f(W) = \mathrm{Diag}\left(M_{shift}^T \hat{z} \tilde{x}_{365}^T\right) + \lambda W^{-1} \log W
    $$
...
&= U^T M_{shift}^T P_{det}^T P_{det} \hat{z} \tilde{x}_{365}^T \\
&= U^T M_{shift}^T z \tilde{x}_{365}^T
\end{align*}
